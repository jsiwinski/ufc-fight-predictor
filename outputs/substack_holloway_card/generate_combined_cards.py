...
Creates 3 PNGs:
- hero_holloway_oliveira.png - Main event only (for thumbnail)
...
def get_odds_for_fight(f1: str, f2: str, odds_map: dict) -> Optional[dict]:
    """Get odds for a specific fight with fuzzy name matching."""
    ...
    for odds_key, fight in odds_map.items():
        parts = odds_key.split('|')
        if len(parts) != 2:
            continue
        o1_norm = normalize_name(parts[0])
        o2_norm = normalize_name(parts[1])
        o1_last = o1_norm.split()[-1] if o1_norm.split() else o1_norm
        o2_last = o2_norm.split()[-1] if o2_norm.split() else o2_norm

        # Match by last names (handles name order swaps, Jr/period issues)
        if (f1_last == o1_last and f2_last == o2_last) or \
           (f1_last == o2_last and f2_last == o1_last):
            return fight

        # Match by all name words (handles Eastern/Western name order)
        f1_words = set(f1_norm.split())
        f2_words = set(f2_norm.split())
        o1_words = set(o1_norm.split())
        o2_words = set(o2_norm.split())
        if (f1_words == o1_words and f2_words == o2_words) or \
           (f1_words == o2_words and f2_words == o1_words):
            return fight

        # Handle single-name fighters (e.g., Sumudaerji)
        if len(f1_norm.split()) == 1:
            if f1_norm in o1_norm or f1_norm in o2_norm:
                if f2_last == o1_last or f2_last == o2_last:
                    return fight
...
async def main():
    ...
    # Load data
    event = load_event_fights("ufc-326-holloway-vs-oliveira-2")
    if not event:
    ...
    hero_html = wrap_in_html(hero_content, "Main Event: Holloway vs Oliveira")
    hero_html_path = OUTPUT_DIR / "hero_holloway_oliveira.html"
    ...
    await capture_html_to_png(hero_html_path, OUTPUT_DIR / "hero_holloway_oliveira.png")
    print("  Captured: hero_holloway_oliveira.png")
