...
Creates 3 PNGs:
- hero_strickland_hernandez.png - Main event only (for thumbnail)
...
OUTPUT_DIR = SCRIPT_DIR  # /mnt read-only, output to project directory
...
async def main():
    ...
    # Load data
    event = load_event_fights("ufc-fight-night-strickland-vs-hernandez")
    if not event:
    ...
    # 0. ALIGNMENT TEST: One card with odds, one without, side by side
    print("\n0. Generating alignment test (with odds vs without odds)...")

    # Find a fight with odds and one without
    fight_with_odds = None
    fight_without_odds = None
    for fight in fights:
        f1, f2 = fight['fighter_1'], fight['fighter_2']
        has_odds = get_odds_for_fight(f1, f2, odds_map) is not None
        if has_odds and fight_with_odds is None:
            fight_with_odds = fight
        elif not has_odds and fight_without_odds is None:
            fight_without_odds = fight
        if fight_with_odds and fight_without_odds:
            break
    ...
    if fight_with_odds and fight_without_odds:
        ...
        test_content = f'''<div class="test-container">
        <div class="test-card">
            <div class="test-label">WITH ODDS</div>
            <div class="fight-list">
                {generate_fight_card_div(fight_with_odds, odds_map, "", False)}
            </div>
        </div>
        <div class="test-card">
            <div class="test-label">WITHOUT ODDS</div>
            <div class="fight-list">
                {generate_fight_card_div(fight_without_odds, odds_map, "", False)}
            </div>
        </div>
        </div>'''
...
{test_content}
        ...
        test_html_path = OUTPUT_DIR / "alignment_test.html"
        with open(test_html_path, 'w') as f:
            f.write(test_html)
        print(f"  Created: {test_html_path}")

        await capture_html_to_png(test_html_path, OUTPUT_DIR / "alignment_test.png", viewport_width=2400)
        print("  Captured: alignment_test.png")
        print(f"  Fight with odds: {fight_with_odds['fighter_1']} vs {fight_with_odds['fighter_2']}")
        print(f"  Fight without odds: {fight_without_odds['fighter_1']} vs {fight_without_odds['fighter_2']}")
    else:
        print("  Skipped - couldn't find both card types")
    ...
    hero_html = wrap_in_html(hero_content, "Main Event: Strickland vs Hernandez")
    hero_html_path = OUTPUT_DIR / "hero_strickland_hernandez.html"
    ...
    await capture_html_to_png(hero_html_path, OUTPUT_DIR / "hero_strickland_hernandez.png")
    print("  Captured: hero_strickland_hernandez.png")
